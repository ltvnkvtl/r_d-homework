...
class TokenController {
  async create(req: Request, res: Response) {
    try {
      ...
      if (!userTokens) {
        ...
        updatedUserTokens = await Token.findByIdAndUpdate(userTokens.id, { $addToSet: { tokens: token } }, { new: true });
      }
  ...
  async updateOne(req: Request, res: Response) {
    try {
      ...
      let updatedUserTokens = await Token.findOneAndUpdate({ userId: id }, { $addToSet: { tokens: token } }, { new: true });
